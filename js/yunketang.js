...
function imgList(){//  导出图片函数  遍历
	...
	$('#imgs').html(strImg);
	$('#idxs').html(strIdx);
	$('article').css('background',imgs[0].bg);//背景变第一张图片颜色   ++++++
}
//console.log(imgList());
imgList();//  调用图片函数
var slider={
	...
	canAuto:true,//能够自动
	init:function(){    //开始函数
		this.canMove();   //this=》  slider
		this.move();
	},
	getN:function(){ ///获得图片 下标
		return $('#imgs li.active').attr('data-num').slice(1); //1-6
	},
	move:function(){//移动图片 函数
		var me=this; //  this> me >slider
		me.canMove();//能够移动的图片函数
		me.timer=setInterval(function(){
			var n=parseFloat(me.getN());// 1
			if(me.canAuto){
				if(n==imgs.length){//先移除第一张效果 然后 在添加第一张的图片 及背景   ++++
...
$('.lbtn').click(function(){
	var n=parseFloat(slider.getN());//1,2,3,4,5,6
	slider.canAuto=false;//   暂停轮播
	if(n>1){ //大于第一张
		$('#imgs li:nth-child('+n+')').removeClass('active');
		$('#idxs li:nth-child('+n+')').removeClass('hover');
		$('#imgs li:nth-child('+(n-1)+')').addClass('active');
...
$('#idxs li').click(function(e){//   点击li  下标------事件
	slider.canAuto=false;
	var m=parseFloat($(e.target).children().html());//3

	var n=parseFloat(slider.getN());/*1-6下标   3*/
	$('#imgs li:nth-child('+n+')').removeClass('active');
	$('#idxs li:nth-child('+n+')').removeClass('hover');
...
majorImgsList();
/*********************微专业课程**********************************************/
var l=parseFloat($('div.zhonglei>ul.kinds').css('left'));  //元素 left的值  0
//console.log(l);
	$('.forward').click(function(){
		if(majorImgs.length-4>(l/-238)){//14-4>0
			l-=238;
...
/***************触发黑幕******************////
var candisplay=true;
$(window).scroll(function(){//触发滚轮
	var free=$('#free').offset().top;//偏移的位置
	//console.log(free);
...
/***************************  验证码******************/
var str="";
function yanzhengma(){//DOM 操作
	var bg=document.createElementNS('http://www.w3.org/2000/svg','rect');//  自定义命名元素  其实就是<rect>
	...
	svg.appendChild(bg);
	var arr='abcdefghijkmnpqrstwxyzABCDEFGHJKLMNPQRSTWXYZ23465789';//52
	var t=document.createElementNS('http://www.w3.org/2000/svg','text');
	t.setAttribute('x',20);
	t.setAttribute('y',25);
	var a=arr[Math.floor(Math.random()*arr.length)];// 5《   《51   
...
$('.modal_login_login').click(function(){
	var result=$('#login-form').serialize();//表单序列化
	console.log(result);
	//if($('[name="yanzhengma"]').val()==str){									
		$.post('data/login.php',result,function(data){   //post请求  请求PHP页面
			if(data.verify==0){
